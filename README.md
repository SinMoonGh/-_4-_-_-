...
여기서 문제에 직면함. 장바구니에 상품을 담을 때는 값이 존재하지만 페이지 이동 시에 값이 없어지는 문제가 생겼습니다.
...
- 따라서 aws ec2 서버를 열고, docker를 설치한 다음 redis를 설치하고, redis이미지로 redis-container를 생성해서 사용하였습니다.


#### deploy.yml file

- Franding 프로젝트에는 올라와 있지 않지만 개인적으로 실습하고 싶어서 만든 파일입니다. Franding 프로젝트 배포에 필요한 모든 작업, 폴더 생성, 라이브러리 설치, 이미지 & 컨테이너 생성까지 이 파일 하나로 해결하고 싶었습니다.
